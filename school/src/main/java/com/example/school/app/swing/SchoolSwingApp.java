...
public class SchoolSwingApp implements Callable<Void> {
	...
	public Void call() throws Exception {
		EventQueue.invokeLater(() -> {
			try {
				Guice.createInjector(new SchoolSwingMongoDefaultModule().mongoHost(mongoHost).mongoPort(mongoPort)
						.databaseName(databaseName).collectionName(collectionName)).getInstance(StudentSwingView.class)
						.start();
			} catch (Exception e) {
